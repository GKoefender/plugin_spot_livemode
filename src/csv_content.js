  ...
  container.innerHTML = `
    <style>
      ...
      #plugin-ui-container ul {
        list-style: none;
        padding: 0;
        max-height: 200px; /* Limitar altura da lista de ads */
...
function handleExportCsv() {
  ...
  const filename = `${campeonato}-${rodada}-${jogo}-${formattedDate}.csv`;

  // Cabeçalho do CSV
  const csvRows = ['Data,Campeonato,Rodada,Partida,Minutagem,Tempo de Exposição,Tipo Propriedade,Propriedade,Cliente,Falha'];
  ...
  data.forEach(row => {
    const values = [
      ...
      row.partida || '',
      row.startTime || '',
      row.endElapsedTime !== null ? row.endElapsedTime : '',
      '', // Tipo Propriedade vazio
      row.property || '',
      row.customer || '',
      row.isFailure ? 'Sim' : 'Não'
